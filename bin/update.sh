...
# Version the custome-ui files
hash=$( md5sum www/custom_ui/state-card-custom-ui.html | cut -c 1-10 )
plain=www/custom_ui/state-card-custom-ui.html
hashed=www/custom_ui/state-card-custom-ui-$hash.html
cp $plain $hashed
perl -pi -e"s{custom_ui/state-card-custom-ui(-\\w+)?.html}{cutom_ui/state-card-custom-ui-$hash.html}" \
    ./configuration.yaml
